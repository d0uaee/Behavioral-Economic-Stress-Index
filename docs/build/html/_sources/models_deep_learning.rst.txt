...
Cette section documente la partie Deep Learning du projet. L'objectif
est de tester si un modele LSTM optimise peut capturer les non-linearites
de la relation BESI-IPC que les modeles statistiques lineaires
(SARIMA/SARIMAX) ne peuvent pas detecter.

**Motivation issue du test de Granger :** La causalite de Granger n'est
pas significative lineairement (p > 0.62). Cela suggere une relation
...
Les features d'entree sont selectionnees pour eviter tout data leakage :
...
Le meme protocole walk-forward sur deux blocs que les modeles
statistiques est utilise pour une comparaison juste :
...
RMSE = 19.738, soit 12x plus eleve que SARIMAX. Deux raisons :

1. **Rupture structurelle hors distribution :** le modele est entraine
   sur 2017-2021 ou l'IPC monte de ~0.5%/an. En 2022 l'IPC monte
   de 6-8%/an — 12x plus rapide — jamais vu pendant l'entrainement.

2. **Taille du dataset insuffisante :** 60 observations d'entrainement
   sont structurellement insuffisantes pour un reseau LSTM.
...
**asymetrique et instructif** :
   ...
   *"Le LSTM apporte de la valeur sur les periodes de relative stabilite
   ...
   Ce resultat confirme que SARIMAX reste superieur sur les series
   macroeconomiques courtes avec choc structurel."*
...
| Fichier                                      | Description                         |
+==============================================+=====================================+
| ``outputs/reports/lstm_results.csv``         | Resultats par bloc                  |
+----------------------------------------------+-------------------------------------+
